...
- 社交模块：业务功能与 IEP 已有实质能力，但 `ModuleRegistry` 中仍偏实验态，社交故事、对话等入口仍有 coming soon 或占位性质。
- 生活技能模块：已有 5 个游戏与自理任务训练，但没有完整进入 `ModuleRegistry`。
- 认知模块：已有 52 条器材资源和 BRIEF/CRT/CognitiveSelf 三个评估入口，但量表内容是 DRAFT，且没有认知训练游戏闭环。
- IEP 抽象：生产页面仍走硬编码生成函数，策略接口和注册机制没有成为主链。
...
当前 `src/features/assessment/report-routes.ts` 已支持 `brief`、`crt`、`cognitive_self` 报告路由，但 `src/views/Reports.vue` 的选项、统计计数、允许跳转集合没有同步。

影响：认知三量表评估后，用户可能无法从报告中心自然查看对应报告。

建议：这是当前最适合先做的小修复。补齐 Reports 页入口、统计、跳转白名单，并加 catalog 到 report center 的一致性测试。
...
### 5.3 ModuleRegistry 部分陈旧

`src/core/module-registry.ts` 当前只注册 sensory、emotional、social；life_skills、cognitive、resource 等能力没有进入完整模块注册。文件中声明的部分功能路由也与真实 router 不完全一致。

影响：平台级模块状态、入口展示、授权、路由、训练、评估之间缺少统一事实源。

建议：不要一次性改成完全动态路由。先加一致性测试，锁住“模块 catalog、训练入口、评估入口、报告入口、授权状态”的关系，再逐步收口。

### 5.4 IEP 策略主链未完成

`src/core/strategies-init.ts` 中只有 SensoryIEPStrategy 注册，其他策略仍是注释状态。生产 IEP 页面主要依赖硬编码 generator，重构后的策略式生成器还没有成为主链。

影响：多模块 IEP 扩展会持续复制逻辑，后续认知、生活、社交增强时维护成本会上升。

建议：先选一个非感统模块做策略化闭环，不要同时迁移全部模块。
...
包配置与 Electron handler 中仍有默认或泛化的更新 URL 痕迹，且未看到完整代码签名与更新烟测证据。

影响：桌面端发布链路存在上线风险，尤其是自动更新与安装器信任链。

建议：发布前统一 HTTPS 更新源、签名配置、安装/升级/回滚烟测清单。

### 5.10 历史命名与零原生依赖边界

About、缓存名、兼容字段中仍有旧名残留。`better-sqlite3` 虽然当前偏导出脚本用途，但与“零额外原生依赖”的运行路线存在认知冲突。

建议：兼容 ID 不要随意改；用户可见文案和非兼容缓存名可分批清理。`better-sqlite3` 后续应替换为 `sql.js` 脚本或隔离为明确的可选开发工具。

## 6. 优先级建议

### P0：低风险收口，建议马上做

- 补齐报告中心对 `brief`、`crt`、`cognitive_self` 的入口、统计和跳转。
- 加一条 assessment catalog 到 report route / Reports 页的一致性测试。
- 修正 `tests/entitlement-catalog.test.ts` 中 cognitive 状态预期，或明确产品决策后同步测试与 catalog。
...
- 增加聚合验证脚本，例如 `test:core` 或 `verify:release`，把 type-check、核心 node 测试、entitlement 测试串起来。
- 发布前补 HTTPS 更新源、签名产物、安装升级烟测。

### P2：平台化收口

- 统一模块、训练、评估、报告、授权 catalog 的事实源。
- 修复或退役部分陈旧的 `ModuleRegistry` 声明。
- 将 dev/migration 路由从生产构建中排除。
- 逐步迁移 IEP 到策略机制，避免继续堆硬编码 generator。

### P3：内容与性能增强

- 做一个完整认知闭环：可信评估 -> 推荐/任务 -> 训练记录 -> 报告。
- 认知量表进入专业审核与常模策略，不把 DRAFT 包装成标准量表。
- 基于实际 profile 决定 DB Worker / Image Worker 是否进入主链。

## 7. 建议的下一步执行顺序

1. 先修报告中心认知三量表入口。这是小面、确定、直接改善体验的 P0。
2. 同步修 entitlement 测试或 catalog 状态，让验证恢复稳定。
3. 给 AI 当前改动补最小验收清单，确认哪些能进主线，哪些仍是实验态。
4. 做资源备份恢复真机演练，形成可复现验收记录。
5. 再开 ModuleRegistry / catalog 统一专题，避免在脏工作区里做大规模重构。
