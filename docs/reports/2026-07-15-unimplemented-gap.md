...
| A4 | 资源文件生命周期未收口 | **Phase 1 已完成（代码）**：删/替即清物理文件（`hardDeleteResource`/`updateResource` 钩入 + 跨表引用计数防线）+ 统一托管路径工具 `resource-file-refs.ts`/`resource-file-service.ts` + AI 候选图 regenerate 清理 + 删 link A 死代码（`ResourceUpload.vue`/`SAVE_ASSET` IPC）；type-check ✅，真机待验。Phase 2（备份纳入物理文件 zip 归档）/ Phase 3（孤儿 GC）待续 | AGENTS §3 技术债；计划 `docs/plans/2026-07-15-a4-resource-file-lifecycle-plan.md` | 🔄 Phase1 代码完成待真机 |
