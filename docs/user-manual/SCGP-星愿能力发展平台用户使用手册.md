...
- 评估焦点、学生与训练等数据概览。
- 首页数据自动保持最新：切回本页时立即刷新，停留期间每 3 分钟静默更新一次，无需手动刷新。
- 训练进度概览卡片：可切换查看近 7 天或近 30 天，含每日训练次数与时长趋势、参与学生数、完成率与各模块的训练分布；点击图上某一天可查看当日训练明细。
...
> [图 S011A] 首页「训练进度概览」卡片的窗口切换、训练次数与时长趋势与模块分布。
