...
后续各波次新小游戏默认按以下显示与交互前提制作：

- 默认运行场景是 `27` 寸触摸大屏，且以**横向优先**布局为准，不按手机竖屏优先设计
- 主视觉、主要操作按钮、状态信息区要为站立触摸使用优化，优先保证大点击面积、远距离可读性和清晰的左右分区
- 进入游戏后默认切换到**全屏沉浸式游戏壳**，隐藏平台主导航、顶部栏和非必要框架干扰
- 新游戏默认采用“大屏横向舞台”心智：
  - 主交互区位于中间或中偏左
  - 说明与主按钮位于右侧或右下可稳定触达区域
  - 状态/HUD 位于左侧或左上独立信息区
- 移动端与小屏只作为兼容兜底，不作为首要视觉基线
- 如果某个游戏因玩法天然更适合其他布局，必须在该游戏实现前单独说明，不默认偏离本前提
