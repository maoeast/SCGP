<!--
  ...
  新路由: /assessment/unified/conners-trs/:studentId
  驱动器: src/strategies/assessment/ConnersTRSDriver.ts
...
<template>
  <div class="conners-trs-assessment">
    <!-- 欢迎对话框 -->
    <el-dialog
    ...
    >
      <div class="welcome-content">
        <h2>Conners 教师用问卷 (TRS)</h2>
        <p class="welcome-intro">本问卷用于评估3-17岁儿童在学校中的行为表现，由教师根据孩子最近6个月在学校的情况填写。共28题，包含品行问题、多动、注意力缺陷和多动指数四个维度。</p>

        <div class="welcome-sections">
          <div class="welcome-section">
            <h4><span class="section-icon">📋</span> 评分说明</h4>
            <p>请根据学生<strong>最近6个月</strong>在学校中的实际表现进行评分：</p>
            <ul>
          ...
          <div class="welcome-section">
            ...
            <p>约5-10分钟，请确保在安静的环境下进行评估。</p>
          </div>

          <div class="welcome-section">
            <h4><span class="section-icon">👨‍🏫</span> 评估人</h4>
            <p>建议由最了解该学生的班主任或主要任课教师填写。</p>
          </div>

          <div class="welcome-section">
            <h4><span class="section-icon">💡</span> 温馨提示</h4>
            <p>请根据学生在学校的实际表现客观填写。如果遇到不确定的题目，可以根据您对该学生的整体印象作答。</p>
          </div>
        </div>

        <p class="welcome-footer">感谢您的配合，这将帮助我们更好地了解和支持学生！</p>
      </div>
      <template #footer>
...
import { connorsTRSQuestions, type ConnersTRSQuestion } from '@/database/conners-trs-questions'
import { calculateConnersScores } from '@/database/conners-scoring'
import { ConnersTRSAPI, ReportAPI } from '@/database/api'
...
// 题目列表
const questions = connorsTRSQuestions
...
const submitAssessment = async () => {
  try {
    ...
    const resultId = trsAPI.createAssessment({
    ...
    try {
      ...
      const title = `Conners教师问卷报告(TRS)_${student.value.name}_${new Date().toLocaleDateString()}`
    ...
    // 跳转到报告页面
    router.push(`/assessment/conners-trs/report/${assessId.value}`)
  } catch (error) {
