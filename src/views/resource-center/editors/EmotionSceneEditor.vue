<template>
  <div class="emotion-editor">
    <el-alert
      ...
      show-icon
      title="情绪与场景可视化编辑器"
      description="当前阶段先固化基础字段、颜色映射和数据契约；题目、选项、反馈的细化编辑会在 Phase 7 完成。"
    />
    ...
    <div class="editor-grid">
      ...
      <el-form-item label="目标情绪" class="editor-item">
        <el-select
      ...
      <el-form-item label="提示上限" class="editor-item">
        <el-select
          :model-value="modelValue.recommendedHintCeiling"
          clearable
          style="width: 100%"
          placeholder="按默认策略"
          @update:model-value="updateField('recommendedHintCeiling', $event)"
        >
          <el-option v-for="level in [0, 1, 2, 3]" :key="level" :label="`级别 ${level}`" :value="level" />
        </el-select>
      </el-form-item>
    </div>
    ...
    <el-card class="contract-card" shadow="never">
      ...
      <div class="summary-grid">
        <div class="summary-item">
          <span class="summary-label">情绪选项</span>
          <strong>{{ modelValue.emotionOptions.length }}</strong>
        </div>
        <div class="summary-item">
          <span class="summary-label">视觉线索</span>
          <strong>{{ modelValue.emotionClues.length }}</strong>
        </div>
        <div class="summary-item">
          <span class="summary-label">推理问题</span>
          <strong>{{ modelValue.prompts.length }}</strong>
        </div>
        <div class="summary-item">
          <span class="summary-label">解决方案</span>
          <strong>{{ modelValue.solutions.length }}</strong>
        </div>
      </div>
      ...
      <el-alert
        ...
        :closable="false"
        show-icon
        title="复杂嵌套项已保留在当前数据模型中"
        description="本阶段先保证现有 prompts / solutions / clues / emotionOptions 的加载、归一化和保存稳定，下一阶段再开放增删与细化编辑。"
      />
    </el-card>
  </div>
</template>
...
import type { EmotionalBaseEmotion, EmotionSceneResourceMeta } from '@/types/emotional'
