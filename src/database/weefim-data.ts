...
export function getWeeFIMLevelDescription(score: number): string {
  if (score >= 126) return '完全独立（所有项目均能完全独立完成，无需任何辅助）';
  if (score >= 108) return '基本独立（能独立完成绝大部分项目，仅个别项目需辅助器具或额外时间）';
  if (score >= 90) return '极轻度依赖（绝大多数项目能独立完成，仅个别较复杂项目需轻微协助或提示）';
  if (score >= 72) return '轻度依赖（多数项目能独立完成，部分项目需最小至中度协助）';
  if (score >= 54) return '中度依赖（约半数项目需中度协助，部分项目需较多协助）';
  if (score >= 36) return '重度依赖（多数项目需大量协助，仅少数简单动作能部分自主完成）';
  return '极重度依赖（几乎所有日常生活与认知任务都完全依赖他人照顾）';
}
