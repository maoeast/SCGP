...
export interface ScoreAdapter {
  ...
  scoreNote: string
  /**
   * 单行 DB 记录 → 归一化快照（与 getLongitudinalScores 完全同一口径）。
   *
   * 供批量场景（首页看板评估缺口分析）直接归一化「每生每量表最新一行」，
   * 避免为每个学生逐量表跑一次全量纵向查询；两处口径永远一致。
   */
  normalizeRow(row: any): ScoreSnapshot
...
const connersPsqNormalizeRow = (row: any): ScoreSnapshot =>
  normalizeConners(row, CONNERS_PSQ_DIMENSIONS)

const connersPsqAdapter: ScoreAdapter = {
  scaleCode: 'conners_psq',
  scaleName: 'Conners 父母症状问卷（PSQ）',
  scoreNote: connersScoreNote,
  normalizeRow: connersPsqNormalizeRow,
  getLongitudinalScores(studentId: number): ScoreSnapshot[] {
    const rows = new ConnersPSQAPI().getStudentAssessments(studentId) ?? []
    return rows.map(connersPsqNormalizeRow).sort(byDateAsc)
  },
}

const connersTrsNormalizeRow = (row: any): ScoreSnapshot =>
  normalizeConners(row, CONNERS_TRS_DIMENSIONS)

const connersTrsAdapter: ScoreAdapter = {
  scaleCode: 'conners_trs',
  scaleName: 'Conners 教师评定量表（TRS）',
  scoreNote: connersScoreNote,
  normalizeRow: connersTrsNormalizeRow,
  getLongitudinalScores(studentId: number): ScoreSnapshot[] {
    const rows = new ConnersTRSAPI().getStudentAssessments(studentId) ?? []
    return rows.map(connersTrsNormalizeRow).sort(byDateAsc)
  },
}
...
const srs2NormalizeRow = (row: any): ScoreSnapshot => normalizeByConfig(row, srs2Config)
const srs2Adapter: ScoreAdapter = {
  ...
  normalizeRow: srs2NormalizeRow,
  getLongitudinalScores(studentId: number): ScoreSnapshot[] {
    const rows = new SRS2AssessmentAPI().getStudentAssessments(studentId) ?? []
    return rows.map(srs2NormalizeRow).sort(byDateAsc)
  },
}
...
const sdqNormalizeRow = (row: any): ScoreSnapshot => normalizeByConfig(row, sdqConfig)
const sdqAdapter: ScoreAdapter = {
  ...
  normalizeRow: sdqNormalizeRow,
  getLongitudinalScores(studentId: number): ScoreSnapshot[] {
    const rows = new SDQAssessmentAPI().getStudentAssessments(studentId) ?? []
    return rows.map(sdqNormalizeRow).sort(byDateAsc)
  },
}
...
const cbclNormalizeRow = (row: any): ScoreSnapshot => normalizeByConfig(row, cbclConfig)
const cbclAdapter: ScoreAdapter = {
  ...
  normalizeRow: cbclNormalizeRow,
  getLongitudinalScores(studentId: number): ScoreSnapshot[] {
    const rows = new CBCLAssessmentAPI().getStudentAssessments(studentId) ?? []
    return rows.map(cbclNormalizeRow).sort(byDateAsc)
  },
}
...
const briefNormalizeRow = (row: any): ScoreSnapshot => normalizeByConfig(row, briefConfig)
const briefAdapter: ScoreAdapter = {
  ...
  normalizeRow: briefNormalizeRow,
  getLongitudinalScores(studentId: number): ScoreSnapshot[] {
    const rows = new BRIEFAssessmentAPI().getStudentAssessments(studentId) ?? []
    return rows.map(briefNormalizeRow).sort(byDateAsc)
  },
}
...
const weefimNormalizeRow = (r: any): ScoreSnapshot => ({
  assessId: r.id,
  date: r.created_at,
  ageMonths: 0,
...
const weefimAdapter: ScoreAdapter = {
  ...
  normalizeRow: weefimNormalizeRow,
  getLongitudinalScores(studentId: number): ScoreSnapshot[] {
    const rows = new WeeFIMAPI().getStudentAssessments(studentId) ?? []
    return rows.map(weefimNormalizeRow).sort(byDateAsc)
  },
}
...
const cnbsr2016NormalizeRow = (row: any): ScoreSnapshot => normalizeByConfig(row, cnbsr2016Config)
const cnbsr2016Adapter: ScoreAdapter = {
  ...
  normalizeRow: cnbsr2016NormalizeRow,
  getLongitudinalScores(studentId: number): ScoreSnapshot[] {
    const rows = new Cnbsr2016AssessmentAPI().getStudentAssessments(studentId) ?? []
    return rows.map(cnbsr2016NormalizeRow).sort(byDateAsc)
  },
}
...
const gmfm88NormalizeRow = (row: any): ScoreSnapshot => normalizeByConfig(row, gmfm88Config)
...
const tgmd3Adapter: ScoreAdapter = {
  ...
  normalizeRow: tgmd3NormalizeRow,
  getLongitudinalScores(studentId: number): ScoreSnapshot[] {
    const rows = new Tgmd3AssessmentAPI().getStudentAssessments(studentId) ?? []
    return rows.map(tgmd3NormalizeRow).sort(byDateAsc)
  },
}

// ---------- SM（社会成熟）：只有总分，无维度 ----------
// SM 无独立维度分，无 age_months（按 age_stage 分段）
const smNormalizeRow = (r: any): ScoreSnapshot => ({
  assessId: r.id,
  date: r.created_at,
  ageMonths: 0,
...
const smAdapter: ScoreAdapter = {
  ...
  normalizeRow: smNormalizeRow,
  getLongitudinalScores(studentId: number): ScoreSnapshot[] {
    const rows = new SMAssessmentAPI().getStudentAssessments(studentId) ?? []
    return rows.map(smNormalizeRow).sort(byDateAsc)
  },
}
...
const abcNormalizeRow = (row: any): ScoreSnapshot => normalizeByConfig(row, abcConfig)
const abcAdapter: ScoreAdapter = {
  ...
  getLongitudinalScores(studentId: number): ScoreSnapshot[] {
    ...
    return rows.map(abcNormalizeRow).sort(byDateAsc)
  },
}
...
const atecNormalizeRow = (row: any): ScoreSnapshot => normalizeByConfig(row, atecConfig)
const atecAdapter: ScoreAdapter = {
  ...
  getLongitudinalScores(studentId: number): ScoreSnapshot[] {
    ...
    return rows.map(atecNormalizeRow).sort(byDateAsc)
  },
}

// ---------- 专用函数：CPEP-3（总通过数锚点 + 数组形 domain_results）----------
/** CPEP-3 单行归一化：解析数组形 domain_results，产出能区 DQ / 通过 / 萌发三维度分。 */
const cpep3NormalizeRow = (r: any): ScoreSnapshot => {
...
const cpep3Adapter: ScoreAdapter = {
  ...
  normalizeRow: cpep3NormalizeRow,
  getLongitudinalScores(studentId: number): ScoreSnapshot[] {
    const rows = new Cpep3AssessmentAPI().getStudentAssessments(studentId) ?? []
    return rows.map(cpep3NormalizeRow).sort(byDateAsc)
  },
}
