...
const THEMES: readonly Theme[] = [
  {
    ...
    sunFill: '#ffd460',
    revealTitle: '蓝天正在慢慢露出来',
    badgeCopy: '今天的云层被你擦开啦，晴空巧手徽章已经亮起来了。',
    celebrationLine: '呼，云层都被你擦开啦，蓝天出来咯！',
  },
  {
    key: 'breeze-harbor',
    title: '微风晴湾',
    ...
    badgeCopy: '你把天空里的云层擦亮了，太阳和微风都在对你笑。',
...
const difficultyLabel = computed(() => {
  ...
  if (props.difficulty === 2) return '中等 · 连续拨云'
  return '困难 · 三层厚云'
})
...
const regenLabel = computed(() => {
  ...
  return regenEvents.value > 0
    ? `重新聚云 ${regenEvents.value} 次 / ${Math.round(idleRecoverMs.value / 1000)} 秒`
    : '保持连续擦除，别让云层回来'
})

const revealSubtitle = computed(() => {
  if (phase.value === 'celebrating' || phase.value === 'finished') {
    return '你把底下的蓝天和阳光都擦出来了。'
  }
  if (props.difficulty === 3) {
    return `同一块厚云还需要反复擦亮，当前已经擦开 ${clearedLabel.value}。`
  }
  return `继续做大动作，底下的蓝天已经露出了 ${clearedLabel.value}。`
})
...
function maybeCompleteSession() {
  ...
  helperMessage.value = '云层都被你擦得远远的，蓝天正在发光。'
...
function stepRegen(now: number) {
  ...
  if (
    ...
    if (!regenActive) {
      regenActive = true
      regenEvents.value += 1
      helperMessage.value = '停太久的话，云层会慢慢回来，继续大范围擦一擦吧。'
    }
