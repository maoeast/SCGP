...
const instructionText = computed(() => {
  if (props.difficulty === 3) return '厚厚的冰霜，同一块要反复擦亮三次！'
  if (props.difficulty === 2) return '挥动手臂擦掉冰霜，别停太久哦！'
  return '挥动手臂，擦掉冰霜！'
})
...
const progressHint = computed(() => {
  ...
  if (pct < 30) return '冰霜还很厚，继续用力擦...'
  if (pct < 60) return '越来越亮了...'
  return '快看到太阳了！'
})
