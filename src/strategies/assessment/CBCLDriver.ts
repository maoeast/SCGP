...
export class CBCLDriver extends BaseDriver {
  ...
  getWelcomeContent() {
    return {
      ...
      intro: '可用于初次筛查儿童情绪行为问题，也可定期追踪干预效果，建议每6-12个月评估一次。CBCL通过113道题目全面评估社会能力（活动、社交、学业）和行为问题（8个综合征量表），计算内化问题、外化问题和总问题T分数，是儿童心理健康的全息雷达，深度扫描焦虑、抑郁、退缩、躯体化、违纪、攻击等深层心理与行为问题。',
      sections: [
        {
          icon: '👨‍🏫',
          title: '给专业人员的实操心法',
          items: [
            'CBCL可用于初筛和追踪，建议每6-12个月评估一次：初次怀疑孩子有情绪行为问题时，CBCL可全面筛查社会能力和8个综合征量表。确认问题后，每6-12个月复测一次，观察总问题T分数、内化/外化问题T分数及各综合征量表变化趋势，评估干预效果。',
            '深挖"躯体化"这座冰山：孩子长期头疼、胃痛、恶心，儿科却查不出毛病，这往往是重度抑郁或极度焦虑的生理变异表达。不要把它当成普通装病逃学，要当成心理急症来应对。',
            '警惕安静的"隐形人"：那些在班里从来不惹事、总是一个人缩在角落里的孩子，往往更容易被漏掉。如果量表显示"退缩""躯体不适"高分，必须优先介入。',
            '用T分数变化追踪干预效果：如果一个孩子基线总问题T分数72（临床显著），经过6个月心理干预后降到66，再6个月降到61，这说明干预有效。T分数每降低5-7分都是显著进步。',
            '关注"内化/外化剪刀差"：有的孩子内化问题T=75（焦虑抑郁严重）但外化问题T=50（行为正常），这提示需要重点关注情绪困难而非行为管理。反之亦然。',
            '配合专项量表使用：CBCL是广谱工具。如发现注意力问题或攻击行为突出，建议进一步使用Conners或SDQ；如躯体化或抑郁突出，建议转介专业精神科评估。',
          ],
        },
        {
          icon: '❤️',
          title: '给家长的填表大实话',
          items: [
            'CBCL既能筛查问题，也能追踪进步，建议每半年到一年做一次：第一次填CBCL是为了全面了解孩子在社会能力和情绪行为方面有没有困难。如果发现问题并开始心理干预，建议每半年到一年复测一次，看总问题分数是降了还是升了。',
            '勇敢撕开那些"隐秘的痛"：量表里可能会问到"自残""想死""玩弄生殖器"。请千万不要隐瞒，这是专业医疗环境，您不说，我们可能就会错过挽救孩子生命的最佳时机。',
            '时间刻度是六个月：如果孩子最近刚换学校、经历重大事件，请在填写时备注。我们要评估的是长期心理特质，而不是短暂的适应性危机。回想过去半年的整体常态。',
            '看长期趋势而非单次T分数：一次评估总问题T分数68（临界）不用太焦虑，重要的是半年后复测是降到62（正常）还是升到74（临床）。持续追踪才能看到孩子的真实变化和干预效果。',
            '社会能力和行为问题都很重要：CBCL不仅看问题，也看孩子的社会能力（活动参与、社交、学业表现）。即使行为问题多，如果社会能力好，也是重要的保护因素。',
          ],
        },
      ],
      reminder: {
        ...
        title: '特别提醒',
        content:
          'CBCL可用于初次筛查儿童情绪行为问题，也可定期追踪干预效果（建议每6-12个月评估一次）。本量表结果仅供学校提供心理辅导和危机干预参考，不能作为抑郁症、焦虑症等精神疾病的诊断依据。如总问题T分数≥70（临床显著）且持续存在，或发现躯体化、自伤、自杀意念等高危信号，请务必立即前往正规医院儿童精神科或心理科就诊。',
      },
    }
  }
