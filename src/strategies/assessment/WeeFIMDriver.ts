...
export class WeeFIMDriver extends BaseDriver {
  ...
  getWelcomeContent() {
    return {
      ...
      intro: '用于追踪儿童在日常生活活动中的功能独立性康复进展，建议每3-6个月评估一次。WeeFIM不测"会不会"，只测"做不做"——通过18个日常生活场景（吃饭、穿衣、如厕、移动、认知等），从1分（完全依赖）到7分（完全独立），精准量化孩子在真实生活中的功能独立程度。',
      sections: [
        {
          icon: '👨‍🏫',
          title: '给专业人员的实操心法',
          items: [
            'WeeFIM是功能独立性追踪工具，建议每3-6个月评估一次：WeeFIM计算总分（18-126分）和功能独立性等级，适合定期监测脑瘫、发育迟缓、神经肌肉疾病等儿童的康复进展。每3-6个月评估一次，观察总分增长速度和各项目得分变化趋势。',
            ...
            '用总分增长速度评估康复效果：如果一个脑瘫儿童，第一次评估总分54分（中度依赖），经过3个月OT/PT训练后复测总分62分，再3个月后总分68分，虽然仍在"轻度依赖"区间，但持续增长说明康复有效。',
            '配合S-M量表使用：S-M测"社会适应能力"，WeeFIM测"功能独立性"。脑瘫孩子可能WeeFIM得分低但S-M正常（认知好但运动受限），而智力障碍孩子可能两者都低。两个量表结合使用能全面了解儿童发展状况。',
          ],
        },
        {
          icon: '❤️',
          title: '给家长的填表大实话',
          items: [
            'WeeFIM是看"独立进步"的量表，建议每3-6个月做一次：如果您的孩子有运动障碍、发育迟缓或正在接受康复训练，WeeFIM可以帮您看到具体进步。比如3个月前吃饭需要喂（2分），现在能用勺子自己吃但会洒（4分），这就是看得见的康复效果。',
            ...
            '看长期趋势而非单次分数：一次WeeFIM总分54分（中度依赖）不用太焦虑，重要的是3个月后复测是涨到60分还是保持54分甚至退步。持续追踪才能看到孩子的真实康复轨迹。',
          ],
        },
      ],
      reminder: {
        ...
        title: '特别提醒',
        content:
          'WeeFIM是功能独立性康复追踪工具，建议每3-6个月评估一次以监测儿童日常生活能力康复进展。本量表评估结果仅供学校制定生活自理训练计划参考，不能作为医学残疾鉴定的依据。如发现由于严重生理功能受限（如重度肌张力异常、持续癫痫发作等）导致的功能障碍，请前往正规医院康复科或儿童神经科就诊。',
      },
    }
  }
