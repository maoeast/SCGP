...
export class ConnersPSQDriver extends BaseDriver {
  ...
  getWelcomeContent() {
    return {
      ...
      intro: '用于追踪ADHD（注意缺陷多动障碍）儿童在家庭中的行为表现和干预效果，建议每3-6个月评估一次（药物治疗期间可每月评估）。Conners-PSQ从家庭生活和日常养育视角，通过48道题目评估孩子的多动、冲动、注意力、品行、学习、焦虑等六大维度，计算T分数（平均50，标准差10）和多动指数，追踪行为干预或药物治疗的效果。',
      sections: [
        {
          icon: '👨‍🏫',
          title: '给专业人员的实操心法',
          items: [
            'Conners-PSQ是ADHD行为追踪工具，建议每3-6个月评估一次（药物治疗期每月评估）：本量表通过计算多动指数和各维度T分数，适合定期监测ADHD儿童在家中的行为表现和干预效果。行为干预期每3-6个月评估一次，药物治疗期建议每月评估以调整用药。',
            ...
            '用T分数变化追踪干预效果：如果一个ADHD儿童基线多动指数T分数75（临床显著），经过3个月行为干预后降到68，再3个月降到62，这说明干预有效。T分数每降低5-8分都是显著进步。',
            '配合TRS量表使用：PSQ反映家中表现，TRS反映学校表现。有的孩子在家多动指数T=75但在学校T=55，这种"环境剪刀差"提示需要重点改善家庭管理策略。',
            '药物治疗期密集追踪：如果孩子开始服用ADHD药物（如哌甲酯），建议每月评估一次Conners-PSQ，观察药物起效情况、副作用和剂量调整效果。',
          ],
        },
        {
          icon: '❤️',
          title: '给家长的填表大实话',
          items: [
            'Conners-PSQ是看"行为改善"的量表，建议每3-6个月做一次：如果您的孩子有ADHD或正在接受行为干预/药物治疗，本量表可以帮您看到家里的变化。比如3个月前多动指数T分数72（临床显著），现在65（临界），这就是实实在在的进步。',
            ...
            '药物治疗期请每月评估：如果孩子在服用ADHD药物，医生可能会要求您每月填一次Conners量表，这是为了观察药物效果、调整剂量、监测副作用。请认真填写，这直接关系到孩子的用药安全。',
            '分数变化比绝对值更重要：多动指数T分数60-75之间都属于"问题区间"，具体多少分不是重点。重点是3个月后分数是降了还是升了，降了多少。每降5分都值得庆祝。',
          ],
        },
      ],
      reminder: {
        ...
        title: '特别提醒',
        content:
          'Conners-PSQ是ADHD行为追踪工具，建议每3-6个月评估一次（药物治疗期间可每月评估）以监测儿童在家庭中的行为表现和干预效果。本量表不能用于ADHD诊断，诊断需要专业医生结合临床观察、病史、多方评估综合判断。如多动指数T分数≥70（临床显著）且持续存在，建议前往正规医院发育行为科或儿童精神科就诊。',
      },
    }
  }
