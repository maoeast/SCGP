...
- **Global scope**: `E:/VSC/H5/SIC-ADS/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/VSC/H5/SIC-ADS/.claude/notes/`)
...
2. Glob `E:/VSC/H5/SIC-ADS/.claude/notes/*.md` (if directory exists) — global notes
...
Global (E:/VSC/H5/SIC-ADS/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `E:/VSC/H5/SIC-ADS/.claude/notes/` — works in any directory
