...
| 项 | 值 |
|---|---|
| 连接方式 | **公网直连** `124.220.104.199`（腾讯云公网 IP，主路径） |
| SSH 客户端 | **必须用 Git Bash 的 ssh/scp**：`C:/Program Files/Git/usr/bin/ssh.exe` / `scp.exe` |
| SSH 私钥 | `E:/Downloads/openclaw.pem`（腾讯云密钥，登 ubuntu 用户，免密 sudo） |
| SSH 用户 | `ubuntu` |
| tailscale 备选 | `vm-0-8-ubuntu` / `100.114.108.86`（2026-08-07 实测 relay 异常 + 打洞失败，不可用；如 tailscale status 显示 active 且 ping 通，也可用此通道） |
...
命令里的 `<version>` 取 `package.json` 的 `version` 字段（如 `1.1.1`）。

### 1. 升版本号（如需）

改 `package.json` 的 `"version"`。若改了更新源 URL 或迁移逻辑，同步更新 `electron/handlers/update.js` 和契约测试 `scripts/tests/update-provider-config.test.mjs`。

> **版本号不变**（同版本覆盖部署）时：electron-updater 同版本视为"已是最新"，已装同版本老客户端的自动升级**不会触发**；该场景仅对新装/手动更新生效。明确用户意图后再做。
...
（`npm run build:electron:win` 等价，分两步跑便于定位：build:web 含 sanitize 生产门禁，约 35s；electron-builder 产出安装包。）

产物在 `release/`：`scgp-setup-<version>.exe` + `.exe.blockmap` + `latest.yml`。签名全 skip（不签名，已知取舍）。`release/` 已 gitignore，不污染工作区。
...
### 4. 上传（scp via 公网直连 + Git Bash，免密）

**分文件传，先大后小**（多文件一次传容易中途断连）：

```bash
"C:/Program Files/Git/usr/bin/scp.exe" -i E:/Downloads/openclaw.pem -o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new \
  release/scgp-setup-<version>.exe \
  ubuntu@124.220.104.199:/home/ubuntu/

"C:/Program Files/Git/usr/bin/scp.exe" -i E:/Downloads/openclaw.pem -o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new \
  release/scgp-setup-<version>.exe.blockmap release/latest.yml \
  ubuntu@124.220.104.199:/home/ubuntu/
```

**传输中断处理**：先 `ssh ... "rm -f /home/ubuntu/scgp-setup-<version>.exe"` 删掉残缺文件再重传；上传后用 `ls -la /home/ubuntu/` 核对 exe 字节数与本地 `release/` 完全一致。

**上传后核对大小**（本地 vs 服务器），本地：`node -e "console.log(require('fs').statSync('release/scgp-setup-<version>.exe').size)"`。

### 5. 部署（sudo 移到目标 + chown/chmod）

```bash
"C:/Program Files/Git/usr/bin/ssh.exe" -i E:/Downloads/openclaw.pem -o IdentitiesOnly=yes ubuntu@124.220.104.199 \
...
- **Windows OpenSSH 拒载私钥（bad permissions）**：`E:/Downloads/openclaw.pem` 权限含 `NT AUTHORITY\Authenticated Users` 读权限，系统 ssh 直接忽略该 key。icacls 收紧需要管理员权限（开发机无）。**解法：一律用 Git Bash 的 `C:/Program Files/Git/usr/bin/ssh.exe` / `scp.exe`**（对 key 权限宽容）。不要复制私钥到别处。
- **tailscale 通道不可用**：health check 报 "could not connect to relay"、`tailscale ping` 超时、服务器 rx 0 → 直接切公网直连 `124.220.104.199`，不要反复重试 tailscale。
- **scp 大文件中断**：`Connection to ... closed by remote host. lost connection` → 删除服务器残缺文件（rm -f）后重传；分文件传（exe 单独传）。
