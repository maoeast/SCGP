...
*   **动态路由与鉴权**: `router/index.ts` 将根据 License 授权状态，结合 `ModuleRegistry` 动态加载对应模块的路由表。
*   **业务状态解耦**: `ResourceSelector` 以及各业务首页组件只需获取当前激活的 `module_code`，即可直接从注册表获取所有相关上下文与资源，极大降低了组件之间的耦合度。
...
*   **模块切换与状态隔离**: 通过 `ModuleRegistry` 动态加载当前激活的模块上下文，确保在不同模块的首页间切换时，资源列表、评估入口等能够根据 `module_code` 进行正确的筛选和状态隔离。
