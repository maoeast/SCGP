...
### 3.1 TypeScript 技术债处理原则

*   **全局历史债现实**: 当前仓库仍存在历史 TypeScript 错误面，常规业务迭代不以“全局 `npm run type-check` 清零”为前置条件，也不要求在每次开发中顺带做全仓重构。
*   **安全特区原则**: 新模块或新子域必须建立并维护自己的严格类型检查边界。以 `emotional` 模块为例，开发与验收时必须通过 `npm run type-check:emotional`，要求 0 报错。
*   **童子军原则**: 仅在实际修改历史文件时，顺手修复该文件内可以安全收敛的 TypeScript 错误；不要为了清理无关历史债扩大改动面。
*   **执行口径**: 如果某项需求只落在专属安全特区内，则以该特区的类型检查结果作为本次交付门槛；只有在明确发起全局清债任务时，才处理全局 `type-check` 阻塞。
