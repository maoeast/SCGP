...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$(node ".codex/get-shit-done/bin/gsd-tools.cjs" workstream list --raw)
fi
```
...
```bash
node ".codex/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
