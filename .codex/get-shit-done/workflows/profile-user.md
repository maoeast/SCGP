...
Key references:
- @.codex/get-shit-done/references/ui-brand.md (display patterns)
- @.codex/get-shit-done/agents/gsd-user-profiler.md (profiler agent definition)
- @.codex/get-shit-done/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH=".codex/get-shit-done/USER-PROFILE.md"
...
```bash
cp ".codex/get-shit-done/USER-PROFILE.md" ".codex/get-shit-done/USER-PROFILE.backup.md"
```
...
✓ Stores profile at .codex/get-shit-done/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(node .codex/get-shit-done/bin/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node .codex/get-shit-done/bin/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `.codex/get-shit-done/references/user-profiling.md`
...
Reference: @.codex/get-shit-done/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node .codex/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node .codex/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node .codex/get-shit-done/bin/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to .codex/get-shit-done/USER-PROFILE.md"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at .codex/get-shit-done/USER-PROFILE.md" and jump to step 10.
...
```bash
node .codex/get-shit-done/bin/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /gsd-dev-preferences at .codex/commands/gsd/dev-preferences.md"

**For AGENTS.md profile section (if selected):**

```bash
node .codex/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node .codex/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```
...
```bash
BACKUP_PATH=".codex/get-shit-done/USER-PROFILE.backup.md"
```
...
Your profile:    .codex/get-shit-done/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /gsd-dev-preferences   .codex/commands/gsd/dev-preferences.md
