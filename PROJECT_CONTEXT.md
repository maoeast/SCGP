...
- 首页页头（`首页看板` 标题、副标题与手动「刷新数据」按钮）已于 2026-09-20 移除，首屏直接是 Hero Banner
- 看板上半部分为上下分层：全宽 Hero Banner（左 `您好，{登录用户姓名}！` + 每日暖心语录；右 核心待办提示 + 主行动按钮）
- Banner 下方为 5 张数据卡片组（学生总数 / 待评估提醒 / 今日训练任务 / 本周异常预警 / 已完成计划数），数据来自 `DashboardAPI` 真实聚合
- 每日语录取自 `src/data/quotes.json`，进入首页时随机抽取一条
- 数据刷新已改为自动：每 3 分钟静默刷新一次（页面隐藏时跳过），窗口重新可见时立即刷新；静默刷新不触发全屏加载遮罩，已无手动刷新入口
- 下半部分三张面板（今日训练日程 / 本周异常预警 / 智能特教助理）当前在 `.dashboard-board__stack` 内**纵向堆叠**（每张只显示前 4 条，超量走「查看全部」弹窗）；2026-09-21 记为待重排项（用户反馈观感差）
- 两个面板的判定口径已有独立真源文档：智能特教助理 → `docs/planning/2026-09-21-首页智能特教助理-评估缺口与干预建议.md`；本周异常预警 → `docs/planning/2026-09-21-首页本周异常预警-判定口径.md`（数据源为统一训练主表 `training_session`，非旧表）
...
## 108. 2026-09-02 生活自理缺图/封面全量落地（以图定步 + wave1 + 穿袜子 + 31 封面）

- **「以图定步」原则（用户拍板）**：老库旧图与步骤文案不符/缺图时，不重画旧图，按文件夹现存图片的数量与内容**重写软件步骤**（步数/文案）。本日 11 任务落地：过马路 8→6 / 问路 7→5 / 喝水 8→7 / 超市文案重排 / 表达如厕 6→5 / 穿衣 7→4 / 穿裤 7→4 / 穿鞋 7→5（m6 系列特殊文件名显式路径）/ 刷牙 8→9 / 男小便 7→8（马桶叙事 B1）/ 女小便 9→10（C1：删除无图"擦拭私处/提裤"步骤——用户确认）。commit `501735a`。
- **wave1 入库**：书包 8→6（删"水杯/纸巾"）、扫地 8→7（删"拿来簸箕"、step8 重画墙角归位）、擦桌 7、乘车 8（step3 上车/step6/7 座位一致重画）——28 张 webp 入库，文件名=步骤号 N.webp（`501735a`）。
- **穿袜子 v2 流程（b8da62c）**：7→4 步全新生成（gpt-image-2 图生图；参考"卡通粗描边"图构图 + 手部/物品主体 + 平涂风格）——**零安全拦截**；此前 v1 的 step2（脚部局部描述）被 content safety 拦截。**绕审核心经验：手部/物品为主体构图；脚部/下装局部描述必被拦**（与 §107 局部边界呼应）。
- **31 张任务卡片封面统一重制（1eb42f4 + 修正 de5d240）**：风格=用户指定"温馨儿童绘本"（柔和水彩渲染+细致彩色铅笔+棕色纤细线条+温暖含蓄色调+简洁留白）；**角色与各任务步骤图性别一致**（4 个女生任务=问路/女小便/超市/梳头——红白条纹波波头女孩×3 + 长黑发女孩×1；男孩按老库各系列角色保留特征）；如厕/脱衣类用背影/物品/遮挡构图；31 张 1280×720 webp q85 替换 `*_cover.jpg`（协议层 jpg→webp 同名回落，inventory 零改动）。补偿修正：梳头镜像（保留镜子+gpt-4o 镜像核验"同一人/方向对称"通过）、床铺人物穿模（改站床尾地面）。
- **用户放弃项**：脱上衣（v2 4 步零拦截但终审全部否定）、整理床铺步骤图（旧分镜太离谱）——二者搁置，勿按原模板再生成。
- 工具链已脚本化：`scripts/apimart-image-gen.py`（新增，batch-json 图生图：上传参考图→提交→轮询→下载）、`scripts/qc-vision-audit.py`（gpt-4o 判票，`--checklist` 参数化）、`download-apimart-urls.py`、`apimart-video-gen.py`（闲置）。工作区 G:\SCGP_Rec\AIimages（AIimages/ 出图、covers/ 封面）。
...
## 112. 2026-09-21 首页「训练进度概览」卡片切统一主表 + 结构化（KPI/组合图/模块分布/日明细）

- 数据源从 legacy 表 `training_records` 切到统一训练主表 `training_session`（时间取 `started_at`、时长取 `duration_ms`），**闭环 §111 的遗留观察**；真库实测旧口径覆盖 835/1142 = **73.1%**（漏器材 186 / 情绪小游戏 101 / 认知内联 20），新口径 100%。
- 口径与展示：窗口 = 最近 N 个**本地自然日**（7/30 可切，含今天）＋ 上一等长窗口做环比；分桶在 JS 侧（不用 SQLite `'localtime'`——wasm 时区行为不可依赖且不可单测）；卡片 = 4 张 KPI（次数 / 时长 / 参与学生 / 完成率，含环比）+ 次数-时长组合图 + 模块分布 + 点柱「当日明细」弹层 + 空态（`hasData`，修掉旧实现「数组恒 7 个点故空态永不触发」的死代码）；日程面板进度条分子改 `summary.todayCount`（与卡片同源、窗口无关）。
- 真源与守卫：口径档 `docs/planning/2026-09-21-首页训练进度概览-判定口径.md`；纯函数口径模块 `src/database/training-progress-rules.ts`（窗口/分桶/完成率/环比/模块分布，零别名依赖可 jiti 直跑）；守卫 `scripts/tests/training-progress-contract.test.mjs`（5 组）+ `tests/training-progress.test.ts`（11 场景），已登记进 `test:core:node` / `test:core:ts`。
- 真渲染 harness 11 项断言全过：7/30 天切换数字变化、点柱打开当日明细弹层（含「已中断」态）、弹层可关闭、「查看全部训练记录」入口、空态显示「暂无训练数据」且不渲染图表。
- 手册同步：截图 fixture 新增 `dashboard-trend` 家族（按采集当天相对生成近 14 个自然日会话，只挂 `plans` / `admin-no-schedule` 两个画像）→ S009/S011 重拍（`dashboard-trend-20260921`）→ approvals + promote → docx 就地换图（`word/media/image13.png` / `image15.png`，`--docx` 校验 217/217）；留痕在截图基线档 §4.6。
- 全局约束补充：**1080 视口放不下「新卡片 + 520px 看板」**，首页整页截图里卡片顶部（标题 + 窗口切换）会被裁（采集锚点在看板）；若手册需要卡片完整形态，须为其单独编号（走插行流程，后续编号整体漂移）。
- 未做（已上报用户）：① 异常面板用本地时间串比 UTC 的 `created_at` 做窗口，+08 时区约 8 小时偏移；② 演示数据时间轴整体按「今天」重锚（评估日期等也硬编码，牵一发动全身）。
