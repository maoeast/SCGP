...
**会话摘要**: 完成 CBCL 报告系统专业优化（心理测量反馈修复）。(1) 修复核心逻辑冲突：新增交叉判断逻辑，当总分T<60但存在子项异常（内化/外化/任意因子T≥65）时，显示"存在特定领域需关注"警告而非"整体发展平稳"。(2) 社会能力分数展示优化：卡片上明确标注"原始分"，新增显示T分数（T ≈ 30/35/50），用颜色区分等级，完善边缘状态（level=1）处理。(3) 视觉一致性修复：宽带量表标签使用三分法（T≥70→danger需关注，T≥65→warning边缘，其他→success正常），与图表保持一致。

### 下次会话优先事项

1. **[P1 - 高] CBCL 端到端验证**
   - 测试 CBCL 评估完整流程：选择学生 → 完成社会能力表单 → 完成113题行为评估 → 保存 → 报告生成
   - 验证报告页面正确显示：交叉判断逻辑、社会能力原始分+T分数、标签颜色三分法
   - **重要**: 测试总分正常但子项异常的边界情况
